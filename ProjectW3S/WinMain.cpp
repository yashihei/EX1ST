...
const TCHAR titleName[] = TEXT("MyApp");
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	int answer = MessageBox(NULL, TEXT("フルスクリーンで起動しますか？"), titleName, MB_YESNO | MB_ICONQUESTION);
	if (answer == IDYES)
		fullScreen = true;

	if (!registerMyClass(hInstance))
		return 0;
	if (!createWindow(hInstance, nCmdShow, &hWnd, ScreenWidth, ScreenHeight, fullScreen))
		return 0;

	try {
		auto game = std::make_unique<GameApp>(hWnd, hInstance, fullScreen);
